...
async def run_trial(
    ...
    # Seed round_history with baseline stances at round -1 so that
    # deliberation round 0 can see them via the history window.
    ...
    for round_num in range(N_ROUNDS):
        ...
        for idx, (a, (text, tokens, probs)) in enumerate(zip(agents, round_results)):
            ...
            if stance is None:
                parse_failures += 1
                # Carry forward: use last known stance
                if a.round_stances:
    ...
    return serialize_trial(
        config, agents, total_tokens, wall_time, parse_failures, model_scale
    )
