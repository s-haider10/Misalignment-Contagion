#!/usr/bin/env python3
"""Compare shadow_no_stance_ablation vs. primary_em on the same dataset.

Both use ratio=0.2, qwen-7b-instruct, model_induced, seed=42 — the only
difference is that shadow_no_stance strips the explicit stance prompt
from the Stage III shadow elicitation, leaving the model to volunteer a
position rather than be asked for one directly.

The comparison answers: does removing the explicit stance ask change the
measured private belief?

Outputs:
  plots_tables/shadow_no_stance_vs_primary/<dataset>/
    summary_table.csv
    shadow_stance_hist.png      shadow distribution: no-stance vs own-last
    ...
    pairwise_delta_hist.png     per-(trial,agent) shadow_no_stance - shadow_primary
"""
...
def plot_shadow_hist(df: pd.DataFrame, out: Path) -> None:
        ...
        for j, var in enumerate(variants):
            ...
            if len(vals):
                ax.hist(vals, bins=bins, edgecolor="black", alpha=0.85,
                        color="steelblue" if var == "own-last" else "darkorange")
    ...
    fig.suptitle("Shadow stance distribution: own-last vs. no-stance elicitation",
                 fontsize=12, y=1.00)
...
def plot_ii_srf(df: pd.DataFrame, out: Path) -> None:
    ...
    fig.suptitle("Private-belief metrics: own-last vs. no-stance elicitation", fontsize=12)
...
def plot_pairwise_delta(df: pd.DataFrame, out: Path) -> None:
    ...
    no_st = df[df["variant"] == "no-stance"].set_index(key)["shadow_stance"]
    own = df[df["variant"] == "own-last"].set_index(key)["shadow_stance"]
    common = no_st.index.intersection(own.index)
    diffs = (no_st.loc[common].astype(float) - own.loc[common].astype(float)).dropna()
    if not len(diffs):
        print("  no paired rows for pairwise delta plot")
        return
    fig, ax = plt.subplots(figsize=(8, 4.5))
    bins = np.arange(-6.5, 7.5, 1)
    ax.hist(diffs, bins=bins, edgecolor="black", color="darkorange", alpha=0.85)
    ax.axvline(0, color="k", lw=1, ls="--")
    ax.axvline(diffs.mean(), color="red", lw=1.5,
               label=f"mean shift = {diffs.mean():+.2f}")
    ax.set_xlabel("shadow_no-stance − shadow_own-last (per agent)")
    ax.set_ylabel("count")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-root", default="plots_tables/shadow_no_stance_vs_primary")
    p.add_argument("--shadow", default=None,
                   help="Override shadow_no_stance path")
...
def main() -> None:
    ...
    shadow_path = args.shadow or (
        f"outputs/shadow_no_stance_ablation/{args.dataset}/{args.model_key}/results.jsonl"
    )
