#!/usr/bin/env python3
"""Compare shadow_self_hidden_ablation vs. primary_em on the same dataset.

Both use ratio=0.2, qwen-7b-instruct, model_induced, seed=42 — the only
difference is that shadow_self_hidden feeds the agent only the FINAL round
of the deliberation with stance digits stripped AND no '(you)'
self-attribution. Compared to shadow_no_stance (which shows the full
transcript with '(you)' tags on the agent's own lines), self_hidden
additionally removes self-anchoring: the agent cannot identify which
utterances are its own and must report a stance without that crutch.

The comparison answers: how much of the primary shadow signal is driven
by the agent re-anchoring on its own past statements?

Outputs:
  plots_tables/shadow_self_hidden_vs_primary/<dataset>/
    summary_table.csv
    shadow_stance_hist.png      shadow distribution: self-hidden vs own-last
    ...
    pairwise_delta_hist.png     per-(trial,agent) shadow_self_hidden - shadow_primary
"""
...
def plot_shadow_hist(df: pd.DataFrame, out: Path) -> None:
        ...
        for j, var in enumerate(variants):
            ...
            if len(vals):
                ax.hist(vals, bins=bins, edgecolor="black", alpha=0.85,
                        color="steelblue" if var == "own-last" else "seagreen")
    ...
    fig.suptitle("Shadow stance distribution: own-last vs. self-hidden elicitation",
                 fontsize=12, y=1.00)
...
def plot_ii_srf(df: pd.DataFrame, out: Path) -> None:
    ...
    fig.suptitle("Private-belief metrics: own-last vs. self-hidden elicitation", fontsize=12)
...
def plot_pairwise_delta(df: pd.DataFrame, out: Path) -> None:
    ...
    sh = df[df["variant"] == "self-hidden"].set_index(key)["shadow_stance"]
    own = df[df["variant"] == "own-last"].set_index(key)["shadow_stance"]
    common = sh.index.intersection(own.index)
    diffs = (sh.loc[common].astype(float) - own.loc[common].astype(float)).dropna()
    if not len(diffs):
        print("  no paired rows for pairwise delta plot")
        return
    fig, ax = plt.subplots(figsize=(8, 4.5))
    bins = np.arange(-6.5, 7.5, 1)
    ax.hist(diffs, bins=bins, edgecolor="black", color="seagreen", alpha=0.85)
    ax.axvline(0, color="k", lw=1, ls="--")
    ax.axvline(diffs.mean(), color="red", lw=1.5,
               label=f"mean shift = {diffs.mean():+.2f}")
    ax.set_xlabel("shadow_self-hidden − shadow_own-last (per agent)")
    ax.set_ylabel("count")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-root", default="plots_tables/shadow_self_hidden_vs_primary")
    p.add_argument("--shadow", default=None,
                   help="Override shadow_self_hidden path")
...
def main() -> None:
    ...
    shadow_path = args.shadow or (
        f"outputs/shadow_self_hidden_ablation/{args.dataset}/{args.model_key}/results.jsonl"
    )
