...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--experiment",
        default=None,
        help="Experiment name (reads from outputs/<name>/results.jsonl, writes to outputs/<name>/tables/).",
    )
    p.add_argument(
        "--input",
        default=None,
...
def main():
    ...
    # Resolve input/output from --experiment or explicit flags
    if args.experiment:
    ...
    if run_all or "cr" in metrics:
        ...
        cr.to_csv(os.path.join(output_dir, "cr_table.csv"), index=False)
        print(f"CR table -> {output_dir}/cr_table.csv")

    if run_all or "delta_cc" in metrics:
        dcc = compute_delta_cc_table(df)
        dcc.to_csv(os.path.join(output_dir, "delta_cc_table.csv"), index=False)
        print(f"DCC table -> {output_dir}/delta_cc_table.csv")

    if run_all or "dtw" in metrics:
        dtw_df = compute_dtw_table(df)
        dtw_df.to_csv(os.path.join(output_dir, "dtw_table.csv"), index=False)
        print(f"DTW table -> {output_dir}/dtw_table.csv")

    if run_all or "logprob_ev" in metrics:
        lp = compute_logprob_ev_table(df)
        lp.to_csv(os.path.join(output_dir, "logprob_ev_table.csv"), index=False)
        print(f"Logprob EV table -> {output_dir}/logprob_ev_table.csv")

    if run_all or "semantic" in metrics:
        sem = compute_semantic_table(df, trials)
        sem.to_csv(os.path.join(output_dir, "semantic_table.csv"), index=False)
        print(f"Semantic table -> {output_dir}/semantic_table.csv")
